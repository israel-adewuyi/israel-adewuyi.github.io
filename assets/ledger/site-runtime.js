(() => {
  ...
  const postRow = (post, extra = "") =>
    `<a class="post-row" href="${articleUrl(post)}"><time>${date(post.date)}</time><div><h3>${escape(post.title)}</h3><p>${escape(
      post.description
    )}</p></div><span class="post-row__topic">${escape(post.topic)}</span><span class="post-row__arrow">&nearr;</span>${extra}</a>`;
  const publication = (item) =>
    `<a class="publication" href="${item.url}" target="_blank" rel="noreferrer"><span>${item.year}</span><div><h3>${escape(
      item.title
    )}</h3><p>${escape(item.authors)}</p></div><em>${escape(item.type)} &nearr;</em></a>`;
  const figure = (post) =>
    `<figure class="research-figure"><img src="${assetRoot}${post.image}" alt="${escape(post.caption)}" /><figcaption><span>Figure / ${escape(
      post.topic
    )}</span><span>${escape(post.caption)}</span></figcaption></figure>`;
  const home = () => {
  ...
  const publications = () =>
    `<header class="page-hero page-hero--compact"><p class="prompt">$ cat ./publications.bib</p><h1>Publications</h1></header><section class="section"><span class="section-label">Bibliography / 02 entries</span><div class="section-body"><div class="publication-list publication-list--full">${data.publications
      .map(publication)
      .join("")}</div></div></section>`;
  ...
  const article = () => {
    ...
    const sourceNote =
      document.body.dataset.productionArticle === "true"
        ? ""
        : `<div class="article-source"><span>Migration draft note</span><p>This candidate preserves the real article title, date, topic, figures, and core text. The final migration would carry over the full source Markdown and citations when one visual direction is selected.</p><a href="https://israel-adewuyi.github.io/blog/${post.date.slice(
            0,
            4
          )}/${
            post.slug === "multiple-winning-subnetworks"
              ? "slim-peft"
              : post.slug === "attention-sink"
                ? "attn_sink_evidence"
                : post.slug === "rl-meets-ner"
                  ? "ner_with_rl"
                  : post.slug
    ...
    return `<header class="article-hero"><p class="prompt">$ open ./writing/${escape(post.slug)}</p><p class="article-meta">${date(
      ...
      post.description
    )}</p></header><article class="article"><aside class="article-toc"><span>Contents</span>${post.sections
      .map((section, index) => `<a href="#section-${index}">${String(index + 1).padStart(2, "0")} ${escape(section.heading)}</a>`)
      .join("")}</aside><div class="article-content">${post.sections
      .map(
        (section, index) =>
          `<section id="section-${index}"><h2>${escape(section.heading)}</h2>${section.paragraphs
            .map((paragraph) => `<p>${escape(paragraph)}</p>`)
            .join("")}${section.bullets ? `<ul>${section.bullets.map((bullet) => `<li>${escape(bullet)}</li>`).join("")}</ul>` : ""}${
            index === 0 ? figure(post) : ""
          }</section>`
      )
          ...
          (entries, value) =>
            entries.forEach((entry) => {
              if (entry.isIntersecting) {
                ...
                value.unobserve(entry.target);
              }
            }),
          { threshold: 0.1 }
        )
      : null;
  document.querySelectorAll(".section, .post-row, .publication, .note-list a, .theme-grid article, .bench-note, .report").forEach((item) => {
